...
#[cfg(windows)]
fn load_proxy_configuration(
    config_dir: &Path,
    save_error_handler: SaveErrorHandler,
) -> Result<(Arc<Settings>, Option<Arc<Modder>>)> {
    let settings = Arc::new(Settings::load_config(config_dir)?);
    let modder = if settings.mod_on() {
        ...
        Some(Arc::new(
            Modder::new(mod_settings, max_data).with_save_error_handler(save_error_handler),
        ))
    } else {
        None
    };
...
async fn reload_proxy(
    ...
    let (settings, modder) = load_proxy_configuration(config_dir, save_error_handler)?;
