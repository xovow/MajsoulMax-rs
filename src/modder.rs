use crate::{
    parser::{HEADER_LEN, MessageKind, ParsedMessage},
...
const ANNOUNCEMENT: &str = concat!(
    "<color=#f9963b>作者: Xerxes-2        版本: ",
...
impl Modder {
    ...
    /// Report save failures while preserving local-only request handling.
    pub fn with_save_error_handler(mut self, handler: SaveErrorHandler) -> Self {
        self.save_error_handler = Some(handler);
        self
    }
    ...
    pub async fn apply_live_patch(&self, patch: LiveModPatch) {
        let mut settings = self.mod_settings.write().await;
        settings.apply_live_patch(&patch);
        // An earlier game edit may have overwritten the sidebar's file write
        // before this command acquired the lock. Persist the updated state too.
    ...
    pub async fn modify(
        ...
        let Ok(parsed) = ParsedMessage::decode(&buf, from_client) else {
            return ModifyResult::forward(buf);
        };
        self.modify_parsed(buf, method_name.as_ref(), parsed).await
    }

    pub(crate) async fn modify_parsed(
        &self,
        buf: Bytes,
        method_name: &str,
        ...
        let res = match parsed.kind {
            MessageKind::Notify => self.modify_notify(buf.clone(), parsed.envelope).await,
            MessageKind::Request(_) => self.modify_req(buf.clone(), parsed.envelope).await,
            MessageKind::Response(_) => {
                self.modify_res(buf.clone(), method_name, parsed.envelope)
                    .await
            }
        };
    ...
    async fn modify_res(
        ...
        let msg = match modified_data {
            Some(data) => {
                msg_block.data = data.into();
                envelope(&buf[..HEADER_LEN], &msg_block)
            }
            None => buf,
        };
        Ok(ModifyResult::forward(msg))
    }
            ...
            ".lq.Lobby.changeCharacterSkin" => {
                ...
                inject_msg = Some(envelope(
                    ...
                    &BaseMessage {
                        ...
                        data: update.encode_to_vec().into(),
                    },
                ));
            }
            ".lq.Lobby.addFinishedEnding" => {
                // drop
                return Ok(ModifyResult::dropped());
            }
    ...
    async fn modify_notify(&self, buf: Bytes, mut msg_block: BaseMessage) -> Result<ModifyResult> {
        let method_name = &msg_block.method_name;
        let mut modified_data: Option<Vec<u8>> = None;
        match method_name.as_str() {
...
mod tests {
    ...
    #[tokio::test]
    async fn unmodified_frames_preserve_unknown_envelope_fields() {
        let modder = fresh_modder();
        for (kind, method, from_client) in [
            (0x01, ".lq.NotifyUnmodified", false),
            (0x02, ".lq.Route.heartbeat", true),
            (0x03, "", false),
        ] {
            let mut wire = wrap(kind, method, vec![0x08, 0x01]).to_vec();
            wire.extend_from_slice(&[0x18, 0x01]); // An unknown protobuf field.
            let wire = Bytes::from(wire);
            ParsedMessage::decode(&wire, from_client).unwrap();

            let result = modder
                .modify(wire.clone(), from_client, ".lq.Route.heartbeat")
                .await;
            assert_eq!(result.msg, Some(wire));
            assert!(result.inject_msg.is_none());
        }
    }
    ...
    async fn broadcast_emoji_uses_server_registered_character() {
        ...
        let body = out.msg.expect("应返回消息体");
        let block = BaseMessage::decode(&body[HEADER_LEN..]).unwrap();
