...
The desktop sidebar offers three automatic update check modes:

| Mode | `autoUpdate` in `settings.json` | Behavior |
| --- | --- | --- |
| Check at startup | `"startup"` | Check once at startup, then load the game |
| Check periodically | `"periodic"` | Check at the configured interval; the first check runs after one full interval |
| Disable automatic checks | `"disabled"` | Stop automatic checks |

The interval field is enabled in periodic mode and accepts 1–65535 minutes, with a default of 60 minutes. Its configuration key is `autoUpdateIntervalMinutes`. Changes are saved automatically and adjust the schedule immediately without reloading. Selecting startup mode takes effect on the next launch. Legacy `autoUpdate: true/false` values still select startup checks or disabled checks, respectively.

The manual “Check for updates” button is available in all three modes. Periodic checks are skipped while an update check or proxy reload is in progress. After a periodic or manual check downloads new data, use “Reload configuration and refresh webpage” to apply it.
...
1. Why are the protocol and resource data updated automatically? What if the update fails?
    - `liqi.desc` describes Mahjong Soul protobuf messages; it is used at build time to generate protocol code.
